function [SM, KM, SD, KD, F] = alternativeUpdate(AMweight, AM, KM, ADweight, AD, KD, ...
                                 Y, alpha, beta, gamma)
...
%     F = zeros(size(Y));
    ...
    for iter = 1:NITER
        ...
        % Update SM
        [SM, KM] = miniJob(AMweight, AM, F, alpha, beta);
        [SD, KD] = miniJob(ADweight, AD, F', alpha, beta);
...
%         LSM = DM - SM1;
        ...
        F = sylvester(2 * alpha * LSM + gamma * idM, 2 * alpha * LSD, gamma * Y);
