function [globalPos] = GetGlobalPosition_batch(AM, AD, Y, pIndex, startPos, endPos)
    nm = size(Y, 1); 
    nd = size(Y, 2);
    
%     [pp, ~] = size(pIndex);
    globalPos = zeros(1, endPos - startPos + 1);
    
    bindwidth = [0.01, 0.1, 1, 5, 10, 50, 100];
    
    for curIndex = startPos:endPos
        Y_temp = Y;
       ...
       % compute a set of Gaussian Interaction Profile kernels
        ...
        % make the score of seed disease-miRNA Ys as zero
        for i = 1:nm
            for j = 1:nd
                if Y(i,j) == 1
                    F(i,j) = 0;
        ...
        globalPos(1, (curIndex - startPos + 1)) = (ll2 + 1 + (ll1 - ll2 - 1))/2;
